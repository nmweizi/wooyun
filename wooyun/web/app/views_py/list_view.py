...
def doc_search():
    ...
    if page_ind <0 : page_ind = 1
    res_count = get_search_counts(search_param,DOC_COLLECTION_NAME)
    ...
    #print "page_ind>>>>"+str(page_ind)+">>>>"+str(pcount)  
    if pcount >0 and page_ind <= pcount:
        res = search(search_param,page_ind,num_per_page,DOC_COLLECTION_NAME)
...
def zone_search():
    ...
    if page_ind <0 : page_ind = 1
    res_count = get_search_counts(search_param,ZONE_COLLECTION_NAME)
    ...
    #print "page_ind>>>>"+str(page_ind)+">>>>"+str(pcount)
    if pcount >0 and page_ind <= pcount:
        res = search(search_param,page_ind,num_per_page,ZONE_COLLECTION_NAME)
    return render_template("zone_list.html",
        search_params=search_param,
        result_count=res_count,
        infors = res,
        page_infor = page_infor)